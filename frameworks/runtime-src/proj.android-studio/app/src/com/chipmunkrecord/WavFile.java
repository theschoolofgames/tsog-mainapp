...
public class WavFile
{
	...
	public static void convertChipMunkVoice(String inputFilePath , String outputFilePath){
		try
		{
			//String filename = "rest.wav";
				WavFile readWavFile = openWavFile(new File(inputFilePath));
				readWavFile.display();
				...
				WavFile writeWavFile = newWavFile(new File(outputFilePath), numChannels, numFrames, validBits, sampleRate + sampleRate/3);
...
//		}
		...
		try
		{
			...
			//{
				...
				WavFile writeWavFile = newWavFile(new File("out.wav"), numChannels, numFrames, validBits, sampleRate*2);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
			...
			WavFile writeWavFiles = newWavFile(new File("out2.wav"), 1, 10, 23, 44100);
			double[] buffers = new double[10];
			writeWavFiles.writeFrames(buffers, 10);
			writeWavFiles.close();
		}
		catch (Exception e)
		{
